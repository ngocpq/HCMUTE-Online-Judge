...
namespace ChamDiem
{
    ...
    public class BienDichVaCham
    {
        ...
        public KetQuaThiSinh ChamBai(IInputCham input)
        {
            ...
            kq.Input = input;
            String outFileName = Path.GetFileNameWithoutExtension(input.SourceCodeFile) + ".exe";
            string outFilePath = Path.Combine(Path.GetDirectoryName(input.SourceCodeFile), outFileName);

            kq.KetQuaBienDich = compiler.BienDich(input.SourceCodeFile, outFilePath);
